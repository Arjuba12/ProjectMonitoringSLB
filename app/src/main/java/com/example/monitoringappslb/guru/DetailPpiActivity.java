...
public class DetailPpiActivity extends BaseGuruActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        int ppiId = readIntExtra("PPI_ID");
        int siswaId = readIntExtra("SISWA_ID");
    ...
    private int readIntExtra(String key) {
        if (getIntent() == null || !getIntent().hasExtra(key)) return -1;
    ...
    private void loadPpiById(int ppiId) {
    ...
    private void loadLatestPpiBySiswa(int siswaId) {
        apiService.getPpiSiswa(siswaId).enqueue(new Callback<PpiListResponse>() {
            @Override
